...
| **G49** | **No way to say when you last did a tracked thing.** The counter could only start from the day you created the routine, so somebody adopting the app had to wait a full cycle before "23 days since your last haircut" meant anything. The API accepted a backdated `log_date` all along; nothing in the UI offered it. It also accepted a *future* one, which made `days_since` negative. | ✅ Fixed — "Last done on" in the editor for tracked routines, writing a real log on that date; the API now rejects a future `log_date` |
| **G50** | **A tracked routine read as overdue on the day it was due.** `overdue = days_since >= target`, so "every 2 days" called you late on day 2, the very day you were acting on time. This contradicted D11's own wording ("becomes overdue once that target is passed"), so it was a bug against the spec, not a missing feature. Reported from real use. | ✅ Fixed — three states (`waiting`, `due`, `overdue`); the target day is due, overdue starts the day after |
